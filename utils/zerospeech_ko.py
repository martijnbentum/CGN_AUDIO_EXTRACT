...
def get_ko_textgrids():
    return Textgrid.objects.filter(component__name__in = 'ko')

def make_ko_sentences():
    textgrids = get_ko_textgrids()
    sentences = []
    for textgrid in progressbar(textgrids):
        sentences.extend(textgrid_to_sentence(textgrid))
    return sentences

def make_ko_words(sentences = None):
    words = []
    if not sentences: sentences = make_ko_sentences()
    for sentence in progressbar(sentences):
        for i, word in enumerate(sentence.words):
            words.append(Word(word, sentence, i))
    return words

def make_ko_phonemes(sentences = None):
    phonemes = []
    if not sentences: sentences = make_ko_sentences()
    for sentence in progressbar(sentences):
...
def save_ko_phonemes(phonemes = None):
    header = 'audio_filename\tstart_time\tend_time\tduration\tphoneme'
    header += '\tprevious_phoneme\tnext_phoneme\tspeaker_id\toverlap'
    header += '\tcomp'
    o = [header]
    if not phonemes: phonemes = make_ko_phonemes()
    for phoneme in phonemes:
        o.append(phoneme.line)
    with open('../news_books_phonemes_zs.tsv','w') as f:
        f.write('\n'.join(o))

def save_ko_words(words = None):
    header = 'audio_filename\tstart_time\tend_time\tduration\ttext\tspeaker_id'
    header += '\toverlap\tcomp'
    o = [header]
    if not words: words = make_ko_words()
    for word in words:
        o.append(word.line)
    with open('../news_books_words_zs.tsv','w') as f:
        f.write('\n'.join(o))

def save_ko_sentences(sentences = None):
    header = 'audio_filename\tstart_time\tend_time\tduration\ttext\tidentifier'
    header += '\tspeaker_ids\tcomp'
    o = [header]
    if not sentences: sentences = make_ko_sentences()
    for sentence in sentences:
        o.append(sentence.line)
    with open('../news_books_sentences_zs.tsv','w') as f:
        f.write('\n'.join(o))
...
class Sentence:
    def __init__(self, words, textgrid, index):
        ...
        self.component = textgrid.component.name
        self.index = index
    ...
    def line(self):
        ...
        m += f'\t{self.identifier}\t{self.speaker_ids}\t{self.component}' 
        return m
...
class Phoneme:
    ...
    def line(self):
        ...
        m += f'\t{self.speaker_id}\t{self.overlap}\t{self.sentence.component}'
        return m
